                                 ...
                                 int nx, int ny, int nz, ValueType* coeffs)
{
    ...
    gko::matrix_data<ValueType, IndexType> data{gko::dim<2>(coarse_dp, fine_dp),
                                                {}};
                      ...
                      IndexType index_help)
{
    ...
    auto restrict_explicit =
        gko::share(gko::matrix::Csr<ValueType, IndexType>::create(
            exec->get_master(), gko::dim<2>(coarse_dp_3D, dp_3D)));
    ...
    std::cout << "\n error on restrict explicit/implicit\n"
              << calculate_error(coarse_dp_3D, lend(x_coarse),
                                 lend(x_coarse_ref))
              << std::endl;
    // write(std::cout, lend(x_coarse_ref));
    // write(std::cout, lend(x_coarse));

    // write(std::cout, lend(restrict_explicit));
}
